 ...
 * MapFile関数
 * @brief 
 * 指定したファイルを開いてファイルの情報を取得する
 * @param filepath 対象のテキストファイル
 * @return std::tuple<int, char*, size_t> 
 */
 ...
 * CopyUTF8String関数
 * @brief 
 * 指定された文字列をUTF-8文字列のサイズやタブ文字を判定しながらコピーする
 * @param [out] dst 
 * @param [in] dst_size 
 * @param [in] src 
 * @param [in] src_size 
 ...
 * DrawLines関数
 * @brief 
 * 指定された行をテキストウィンドウに表示する
 * @param lines 描画する行単位の文字列
 * @param start_line 描画開始する行番号
 * @param layer_id テキストウィンドウのレイヤーID
 * @param w 
 * @param h 
 * @param tab 
 */
 ...
 * @brief 
 * キー入力を待つ
 * @return std::tuple<bool, int> 終了かどうか
 */
 ...
 * UpdateStartLine関数
 * @brief 
 * キー入力を待って、入力されたキーに合わせて行表示を更新する
 * @param start_line 開始行番号
 * @param height 表示している行数
 * @param num_lines 最大行数
 * @return true 終了
 * @return false 継続
 */
