...
const array<uint32_t, kSurface.size()> kColor{
  0xff0000, 0x00ff00, 0xffff00, 0x0000ff, 0xff00ff, 0x00fffff
};
...
extern "C" void main(int argc, char** argv) {
  ...
  for (;;) {
    ...
    // 麺中心の Z 座標(を 4倍 した値) を 6 面について計算
 ...
 * @brief 
 * 立方体を描画する
 * @param layer_id 描画するウィンドウのレイヤID
 */
void DrawObj(uint64_t layer_id) {
  // オブジェクト座標 vert を スクリーン座標 scr に変換（画面奥がZ+）
  for (int i = 0; i < kCube.size(); i++) {
...
void DrawSurface(uint64_t layer_id, int sur) {
  ...
  int ymin = kCanvasSize, ymax = 0; // 画面の描画範囲 [ymin, ymaxe
